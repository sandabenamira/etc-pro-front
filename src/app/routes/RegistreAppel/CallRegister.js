...
class CallRegister extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      _.isEmpty(this.props.classesList.classes)
    ) {
    }
  }
  ...
  handleChangeClasse = (name) => (event) => {
    ...
    axios
      .get(
        `${baseUrl.baseUrl}/classes/getListSeances/${JSON.stringify(
          data
    ...
    axios
      .get(
        `${baseUrl.baseUrl}/call_registers/getLastV_Calls?class_id=${event.target.value}&access_token=${localStorage.token}`
      )
      .then((res) => {
        let data = {
  ...
  handleDateChange = (date) => {
    ...
    let lastV_contextE = notEmpty(this.state.last_version)
      ? this.state.last_version.context_event
      : {};

    if (
      notEmpty(lastV_contextE) &&
      moment(date).isSameOrAfter(lastV_contextE.start_lesson, "day")
    ) {
      this.setState({
        date_gEvent: date,
        result: [],
      ...
      notEmpty(lastV_contextE) &&
      moment(date).isBefore(lastV_contextE.start_lesson, "day")
    ) {
      this.setState({
        date_gEvent: date,
        result: [],
  ...
  ModalConfirmSave() {
    ...
    axios
      .post(
        `${
          baseUrl.baseUrl
        }/register_versions/createCallsAndUpdate?student_calls=${JSON.stringify(
          student_calls
        )}&callRegister=${JSON.stringify(
          objCallR
        )}&contextEvent=${JSON.stringify(contextEvent)}&access_token=${
          localStorage.token
        }`
      )
      .then((res) => {
  ...
  SetAllAbsent() {
    ...
    if (this.state.prevStudentStatus) {
      setStudentList = this.state.prevStudentStatus.map((data, index) => ({
        ...data,
        action: "Absent",
      }));
      this.setState({
              ...
              {this.state.alert_success ? (
                <UncontrolledAlert className="alert-addon-card bg-success bg-success text-white shadow-lg">
                  ...
                  <span className="d-inline-block">
                    {" "}
                    {<IntlMessages id="message.success.call" />}
                  </span>
                </UncontrolledAlert>
              ) : (
                ""
                ...
                <div className="row">
                  <div className="col-lg-3 col-sm-6 col-12">
                    <FormControl className="w-100 mb-2">
                      ...
                      >
                        {Array.from(this.props.classesList)
                          .filter((classe) => classe.status)
                          .map((classe) => (
                  ...
                  <div className="col-lg-4 col-sm-6 col-12">
                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                      <DatePicker
                        label={
                          <IntlMessages id="components.call.register.jour" />
                        }
                        fullWidth
                  ...
                  <div className="col-lg-5 col-sm-6 col-12">
                    <FormControl className="w-100 mb-2">
                      <InputLabel htmlFor="age-simple">
                        {
                          <IntlMessages id="components.call.register.sceances" />
                        }
                      </InputLabel>
                      <Select value={this.state.seance}>
                        {Array.from(this.state.list_gEvent).map(
                          (gEvent, index) => {
                            let contextEvent = _.first(gEvent.sameDayContextE);
                            let dynamicBtnClass =
                              !_.isEmpty(contextEvent) && contextEvent.tag_call
                                ? classes.button
                                : "";
                            return (
                              <MenuItem
                              ...
                              >
                                ...
                                {getSubjectName(
                                  Array.from(this.props.subjects),
                                  gEvent.subject_id
                                ) +
                                  " " +
                                  moment(
                                    gEvent.start_date_generic_event
                                  ).format("HH[h]mm") +
                                  "-" +
                                  moment(gEvent.end_date_generic_event).format(
                                    "HH[h]mm"
                                  )}
                              </MenuItem>
                            );
                      ...
                      >
                        ...
                        <span>
                          {": " +
                            moment(
                              this.state.last_version.context_event
                                .generic_events.start_date_generic_event
                            ).format("dddd D MMM") +
                            " " +
                            getSubjectName(
                              Array.from(this.props.subjects),
                              this.state.last_version.context_event
                                .generic_events.subject_id
                            ) +
                            " " +
                            moment(
                              this.state.last_version.context_event
                            ...
                            moment(
                              this.state.last_version.context_event
                                .generic_events.end_date_generic_event
                            ).format("HH[h]mm")}
                        </span>
                      </p>
                    </div>
