...
function EstablishmentItemCard({ data, editEstablishment, moduleList, countries, estabTypes }) {
  const { id, name, phone, website, name_director, surname_director, module_id, countries_id, estab_type_id } = data;
  return (
      ...
      <div className="media-body jr-featured-content">
        <div className="jr-featured-content-left">
          {moduleList.map((element) => {
            ...
            return moduleArray.includes(element.id) ? (
              <span key={element.id} className="jr-tag text-uppercase bg-info d-inline-block" color="#06BB8A">
                {element.name}
              </span>
            ) : (
              ''
          ...
          <div className="d-flex flex-row">
            <p className="text-grey mb-1">
              <i className={`zmdi zmdi-account jr-fs-lg mr-2 d-inline-block align-middle`} />
              {'Directeur: ' + name_director + ' ' + surname_director}
            </p>
          </div>
        </div>
        <div className="jr-featured-content-right">
          <div>
            <h2>
              <a className="mb-0 jr-font-weight-medium" href={`http://${website}`}>
                {website}
              </a>
            </h2>
            <p className="text-grey jr-fs-sm">Tél :{phone}</p>
          </div>
          <p onClick={() => editEstablishment(id)} className="text-primary mt-auto mb-0 pointer">
            <span>{<IntlMessages id="dashboard.viewPlus" />}</span>
